...
class PointCloud:
    ...
    def get_verts_coords(self) -> tuple[float, float, float]:
        x, y, z = np.array(self.verts).T
        return x, y, z

    def get_faces_coords(self) -> tuple[float, float, float]:
...
class Modelnet40DataLoader(object):
    ...
    def __preprocess__(self, file) -> PointCloud:
    ...
    def _read_off_file(self, file) -> PointCloud:
