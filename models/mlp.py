...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, hidden_size: int,
                 hidden_layers: int, device:str, activation: str = "ReLU", from_saved_state_dict : str = None):
        super(MLP, self).__init__()

        self.device = device
        layers = []
        layers_sizes = [(input_dim, hidden_size)] + [(hidden_size, hidden_size)] * hidden_layers
        ...
        for _, (input_size, output_size) in enumerate(layers_sizes):
            ...
            if activation == "Tanh":
                ...
                raise Warning("The value passed as activation type is not none, but no activation with that name was "
                              "found!")
    ...
    def train(self, optimizer, criterion, num_epochs,train_loader, save_path_folder: str = 'saved_models'):
